...
| `rules` | boolean | `false` | 🟢 ACTIVE | 启用 `acp_rule` 工具——记录简短、原则性的持久提醒，永不被压缩。**默认关闭。** |
...
`rules` 功能提供一个普通工具 `acp_rule`，用于记录**必须存活于上下文压缩之外的持久提醒**。它与其他所有 ACP 工具正交——只是留痕：不压缩、不搜索、不触碰会话内容，也不修改系统提示词。

**用法。** 工具只有一个可选参数 `rule`：
- 传入文本 → 记录该条并回显 `Recorded <id>: <text>`；
- 不传（或传空白）→ 列出全部已记录规则，供人查看。

刻意不提供 remove/clear——过期条目保留到会话结束，或直接编辑状态文件清理。

**记什么。** 工具描述会指导模型在以下情况使用 `acp_rule`：
- 用户**特别强调或反复强调**的教训、规则、偏好；
- 用户要求模型**记住某个具体行为**；
- 模型自己撞到的**大坑**，值得不再重蹈覆辙。

条目要求**简短、以原则为主**——每条一行，而非段落。内核对超过 300 字符的条目直接拒收，每个会话最多 50 条，均以纯文本提示而非报错的方式告知。

**为什么规则能扛住压缩。** `acp_rule` 位于内核的 always-protected 集合（与 `compress` 并列），其工具调用和返回默认被排除在压缩范围之外——留痕永远不会从对话记录中被剪掉。规则还会随会话压缩状态一起持久化到 `.acp.json` sidecar 文件中，同一会话重启后依然保留。
...
- **默认值：** `false`
- **状态：** 🟢 ACTIVE
- **说明：** 启用 `acp_rule` 工具。默认关闭，保证既有会话行为不变。
