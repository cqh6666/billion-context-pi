...
- **退出码 0 不等于成功。** 退出码为 0 但没有交付最终答案的运行(无回复文本、且最后可观测动作不是工具执行 — 例如只产出一段 thinking 就停住)同样按 `FAILED ⚠️` 报告并附诊断,绝不会被静默标记为 `completed`;而以工具执行收尾的运行(如 worker 写完文件即结束)仍为 `completed`。
